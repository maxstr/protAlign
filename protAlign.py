...
def tmAlign(file1, file2, tmPath):
    tmPath = join(os.path.dirname(os.path.realpath(__file__)), 'TMalign')
    with settings(capture=True):
        return local("%s %s %s" % (tmPath, file1, file2))
# Extract RMSD from TM-align output
...
def main(seqDirectory, nativePDB, destination):
    ...
    for pair in modelPairs:
        ...
        with open(abspath(join(dataDir, "%s-%s-align.txt" % (model1, model2))), 'w') as f:
            f.write(tmOutput)
    ...
    results['meanModelsTMScore'] = numpy.std(modelsTMScore)
    ...
    for i in models:
        ...
        with open(abspath(join(dataDir, "%s-%s-align.txt" % ('native', i))), 'w') as f:
            f.write(tmOutput)
    ...
    results['meanModelNativeTMScore'] = numpy.std(modelNativeTMScore)
    # Now we find the model with the lowest RMSD, TMScore
    results['minRMSDModel'] = min(modelNativeResultsDict, key=lambda model:ModelNativeResultsDict[model]['rmsd'])
    results['minRMSValue'] = modelNativeResultsDict[minRMSModel]['rmsd']
    results['maxTMScoreModel'] = max(modelNativeResultsDict, key =lambda model:ModelNativeResultsDict[model]['tmScore-1'])
    results['maxTMScoreValue'] = modelNativeResultsDict[maxTMScoreValue]['tmScore-1']
    ...
    TMScore Max (model): %(maxTMScoreValue) ( %(maxTMScoreModel) )
